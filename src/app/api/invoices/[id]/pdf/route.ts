...
export async function GET(
  ...
  try {
    ...
    const billTo = (invoice.contact as any)?.name || (invoice.notes || '').split('\n')[0] || 'Customer'
    ...
    const html = '<!DOCTYPE html>' +
      ...
      '</style></head><body>' +
      '<div class="header">' +
      '<div><div class="business-name">' + business.name + '</div>' +
      ...
      '</tr></thead><tbody>' +
      '<tr><td colspan="4" style="padding:20px 0;color:#999;">No line items</td></tr>' +
      '</tbody></table>' +
      '<div class="totals">' +
      ...
      '</div>' +
      (paymentUrl ? '<div style="margin-top:40px;padding:20px;background:' + primaryColor + '10;border-radius:12px;"><p style="font-size:14px;font-weight:700;color:' + primaryColor + ';margin-bottom:8px;">Pay Online</p><a href="' + paymentUrl + '">' + paymentUrl + '</a></div>' : '') +
